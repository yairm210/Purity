...
object ExpectedFunctionPurityChecker {
    ...
    private fun isAcceptableGetter(function: IrFunction): Boolean {
        val body = function.body ?: return true

        // From outside our code, we can't know - assume it's ok, since most getters are autogenerated
        if (body is IrSyntheticBody) return true

        // Autogenerated getter - return field
        if (body.statements.size == 1){ 
            val statement = body.statements.single()
            if (statement !is IrReturn) return false // I'm pretty sure this is impossible - how can a getter have 1 statement that doesn't return?
            val returnValue = statement.value
            if (returnValue is IrGetField) return true
        }
        return false
    }
