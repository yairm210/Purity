    ...
    fun isMarkedAsPure(function: IrFunction, purityConfig: PurityConfig): Boolean {
        ...
        if (function is IrSimpleFunction) {
            for (overriddenFunction in getAllOverriddenFunctions(function).toSet()) {
                ...
                if (overriddenFunctionName in wellKnownPureFunctions) return true
                if (overriddenFunctionName in purityConfig.wellKnownPureFunctionsFromUser) return true
                if (wellKnownPureFunctionsPrefixes.any { overriddenFunctionName.startsWith(it) }) return true
                if (overriddenFunction.hasAnnotation(Annotations.Pure)) return true
            }
        }
